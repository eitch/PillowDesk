...
public class RoomResource {
	...
	public Response getRooms(@Context HttpServletRequest request) {

		Certificate cert = getCertificate(request);
		try (StrolchTransaction tx = openTx(cert)) {
			List<Resource> rooms = tx.getResourceMap().getElementsBy(tx, TYPE_ROOM);
			return ResponseUtil.toResponse(rooms, r -> r.accept(new StrolchRootElementToJsonVisitor()));
		}
	}
}
